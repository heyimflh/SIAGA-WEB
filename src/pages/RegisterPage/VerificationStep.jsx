...
//     for client at all (Requirement 8.6a — must not be hidden via CSS).
...
export default function VerificationStep({ state, dispatch }) {
  ...
  async function handleSubmit(e) {
    ...
    // (Requirement 14.2 / Property 10) and surface the banner copy
    // (Requirement 14.2a).
    dispatch({
  ...
  return (
    <form className="verification-step" onSubmit={handleSubmit} noValidate>
      ...
      {/* Role branch — Requirement 8.6a: client must NOT render SidopiUpload. */}
      {state.role === 'pilot' ? (
